;(function(exports) {
  exports.ok = function(val, message) {
    if(!val) {
      throw new Error(message || ('not ok: ' + val))
    }
  }

  exports.equal = function(lhs, rhs, message) {
    if(lhs != rhs) {
      throw new Error(message || ('lhs ("'+lhs+'") should == rhs ("'+rhs+'")'))
    }
  } 
  ...
  exports.deepEqual = function(lhs, rhs, message) {
    if(!is_deep_equal(lhs, rhs)) {
      throw new Error(message || ('lhs ("'+lhs+'") should deep equal rhs ("'+rhs+'") '))
    }
  }

  exports.notDeepEqual = function(lhs, rhs, message) {
    if(is_deep_equal(lhs, rhs)) {
      throw new Error(message || ('lhs ("'+lhs+'") should not deep equal rhs ("'+rhs+'") '))
    }
  }
  ...
  function _throws (should_throw, block, expected, message) {
    ...
    if(should_throw && !actual) {
      throw new Error('Missing expected exception ' + (message || ''))
    } else if(!should_throw && actual) {
      throw new Error('Got unwanted exception ' + actual + (message || ''))
    }
  }
